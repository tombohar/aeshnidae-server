...
///     assembly "Aeshnidae.DatBackup.dll"  ->  type "Aeshnidae.DatBackup.Mod"
...
{
    /// <summary>
    /// "Aeshnidae.DatBackup". Taken from the assembly rather than written out, so a
    /// rename cannot leave a stale literal behind.
    /// (Note: nameof(Aeshnidae.DatBackup) would yield just "Template" - nameof on a
    ...
    /// <summary>Folder this mod was loaded from, e.g. C:\ACEPublic\Mods\Aeshnidae.DatBackup</summary>
    ...
    /// </summary>
    /// <summary>
    /// The startup snapshot, off the initialisation thread.
    ///
    /// Hashing 1.4 GB of dats takes seconds, and Initialize() runs while the server
    /// is still coming up - blocking it would delay the world opening for a backup
    /// that nothing is waiting on. Failures are logged and dropped for the same
    /// reason: a server that will not start because a backup failed is a worse
    /// outcome than a server with one missing snapshot.
    /// </summary>
    private static void StartupSnapshot()
    {
        Task.Run(() =>
        {
            try
            {
                var source = Settings.ResolvedDatDirectory;
                var store = Settings.ResolvedBackupDirectory;

                var (snapshot, message) = SnapshotStore.Take(
                    store, source, "server startup", Settings.KeepSnapshots,
                    m => ModManager.Log($"[{Name}] {m}"));

                if (snapshot is null)
                {
                    ModManager.Log($"[{Name}] {message}", ModManager.LogLevel.Warn);
                    return;
                }

                if (!Settings.VerifyOnStartup)
                    return;

                var (ok, problems) = SnapshotStore.Verify(store, snapshot);

                if (ok)
                    ModManager.Log($"[{Name}] verified snapshot {snapshot.Id}");
                else
                    foreach (var p in problems)
                        ModManager.Log($"[{Name}] verify: {p}", ModManager.LogLevel.Error);
            }
            catch (Exception ex)
            {
                ModManager.Log($"[{Name}] startup snapshot failed: {ex.Message}", ModManager.LogLevel.Error);
            }
        });
    }
